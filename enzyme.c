...
void *run_enzyme(void *data) {
	...
	*/
	thread_info_t* ptr=(thread_info_t*)data;
	ptr->swapcount=0;
	...
	while(please_quit==0)
	{
		if(ptr->string[0]>ptr->string[1])
		{
			workperformed=1;
			ptr->swapcount++;
			char t=ptr->string[0];
			ptr->string[0]=ptr->string[1];
			ptr->string[1]=t;
		}
		if(use_yield!=0)
		pthread_yield();
...
// The fourth argument is used to pass arguments to the function, fp(run_enzym).
	    if (rv) {
...
int join_on_enzymes(pthread_t *threads, int n) {
	...
	int data=0; // just to make the code compile 
    ...
    // thread_return: pointer to the location where the exit status of the
  ...
  if(rv) {
	...
	if ((void*)status == PTHREAD_CANCELED) { //After a canceled thread has terminated, 
	    ...
	    printf("Thread %d did not return anything\n",i);
	      ...
	      int threadswapcount = data; 
	      // Hint - you will need to cast something.
		  thread_info_t* t=(thread_info_t*)status;
          threadswapcount+=t->swapcount;
...
int smp2_main(int argc, char **argv) {
	...
	pthread_t enzymes[MAX]; //store threads
	int n,totalswap;
	char string[MAX];
	...
	please_quit = 0; //extern int
	use_yield =1;  //global int
	...
	pthread_t sleeperid;
	//int pthread_detach(pthread_t sleeperid;);
