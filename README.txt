...
synchronization error that could occur in this code. Be specific.
 ...
 Ans: In the multithread programming, all threads will run at the same time without lock 
 so when a variable is used in the adder function but it also need to be updated by multiply, 
 which will cause synchronization error.  
...
Why?
   Ans: all c string needed to store the operations such as buffer[],S[]
   should be protected because they are frequently updated and read.

Q4: What would happen if you had a busy-wait within one of your critical
sections?  What if it is a loop with sched_yield()?

  Ans:A loop with sched_yield can let other threads to execute but a busy-wait just 
  to check the conditions of other threads.
...
Ans: If we need multiple mutexes in a program, it is easier to be deadlock so in order to 
solve this kind of situation, we should use trylock since it can exit failure if the 
...
correctly implement this behavior?  Why or why not?
 ...
 Ans: yes, it is necessary to have a mutex in order to have the correct behaviour
 of counting since multiple threads can access to the num_ops at the same time ,
...
Ans: In order to improve the efficiency of our programming, we should keep the critical
selection as small as possible. which can optimize the cocurrency of the program.
